...
제일 처음에 개발을 접한 건 초등학교 저학년 때 학교에서 방과 후에 홈페이지 만드는 거 교육해 준다고 해서 처음 나모 웹에디터를 통해서 접한 게 처음이었던 것 같아요. 또 제가 게임을 되게 좋아해서 게임을 만들고 싶었어요. 그런데 집에서는 제가 컴퓨터, 특히 게임하는 걸 별로 안 좋아해서 컴퓨터를 할 거면 발전적인 걸 하라고 하셔서 컴퓨터 자격증 이런 걸로 컴퓨터를 했죠. 근데 고등학교 때 까지도 정작 게임 개발은 한 번도 못 해봤어요.
